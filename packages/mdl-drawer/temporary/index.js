...
import {remapEvent, getTransformPropertyName, supportsCssCustomProperties} from '../util';
...
export default class MDLTemporaryDrawer extends MDLComponent {
  ...
  getDefaultFoundation() {
    ...
    return new MDLTemporaryDrawerFoundation({
      ...
      registerInteractionHandler: (evt, handler) => this.root_.addEventListener(remapEvent(evt), handler),
      deregisterInteractionHandler: (evt, handler) => this.root_.removeEventListener(remapEvent(evt), handler),
      registerDrawerInteractionHandler: (evt, handler) => this.drawer.addEventListener(remapEvent(evt), handler),
      deregisterDrawerInteractionHandler: (evt, handler) => this.drawer.removeEventListener(remapEvent(evt), handler),
